...
export class RRWindow {
  ...
  constructor(classification: string, title: string, json: object) {
    ...
    this._widgets = this._getWidgets(arr);
  }
  ...
  _getWidgets(rideArr: RideRequirement[]): WidgetBase[] {
    ...
    const listViewW: ListViewWidget = {
      ...
      items: rideArr.map(r => r.name),
      onClick: (item: number, _column: number) => {
        if (this._handle) {
          const ride = rideArr[item];
          // name
